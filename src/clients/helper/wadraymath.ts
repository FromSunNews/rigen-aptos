...
const DEFAULT_DECIMALS_DISPLAY = 2;
...
declare module "@ethersproject/bignumber" {
  interface BigNumber {
    ...
    negated: () => BigNumber;
    rayToPercent: () => number;
    formatRayToNumber: ({ displayFixedUnit }?: { displayFixedUnit?: number }) => number;
    formatToNumberWithDecimals: ({
      decimals,
      displayFixedUnit,
    }?: {
      decimals?: number;
      displayFixedUnit?: number;
    }) => number;
  }
}
...
BigNumber.prototype.rayToPercent = function (): number {
  return Number(this.div(BigNumber.from(10).pow(25))) / 100;
};

BigNumber.prototype.formatRayToNumber = function (
  { displayFixedUnit }: { displayFixedUnit?: number } = { displayFixedUnit: DEFAULT_DECIMALS_DISPLAY }
): number {
  const formatted = formatUnits(this, 27);
  return Number(Number(formatted).toFixed(displayFixedUnit));
};

BigNumber.prototype.formatToNumberWithDecimals = function (
  { decimals = 8, displayFixedUnit = -1 }: { decimals?: number; displayFixedUnit?: number } = {
    decimals: 8,
    displayFixedUnit: DEFAULT_DECIMALS_DISPLAY,
  }
): number {
...
// OR you can use this class to wrap the BigNumber methods
export class BigNumberWrapper {
  ...
  public rayToPercent(a: BigNumber): number {
    return a.rayToPercent();
  }

  public formatRayToNumber(a: BigNumber, { displayFixedUnit }: { displayFixedUnit?: number }): number {
    return a.formatRayToNumber({ displayFixedUnit });
  }

  public formatToNumberWithDecimals(
    a: BigNumber,
    { decimals, displayFixedUnit }: { decimals?: number; displayFixedUnit?: number }
  ): number {
    return a.formatToNumberWithDecimals({ decimals, displayFixedUnit });
  }
}

export const bn = new BigNumberWrapper();
