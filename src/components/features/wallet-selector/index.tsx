...
import { CustomImage, Source, ImageFallback } from "@/components/shared/ui/custom-image";
...
export function WalletSelector(walletSortingOptions: WalletSortingOptions) {
  ...
  const { isWalletOpen, setWalletModalOpen } = useBoundStore();
    ...
    <Dialog>
      <DialogTrigger asChild>
        <Button variant="secondary" className="rounded-full px-3 py-4">
          ...
          <Typography>{account?.ansName || truncateAddress(account?.address) || "Unknown"}</Typography>
      ...
      <DialogContent className="sm:max-w-[465px]">
        ...
        <div className="flex flex-col items-center gap-4 pb-2 pt-4">
          ...
          <div className="flex cursor-pointer flex-row items-center gap-2 rounded-full border border-ring/30 px-3 py-2">
            <Typography>{account?.ansName || truncateAddress(account?.address) || "Unknown"}</Typography>
            <FaRegCopy size={18} onClick={copyAddress} />
          </div>
          ...
          <div className="flex w-full flex-col">
            <Link href="/" className="flex flex-row gap-2 rounded-md p-2 hover:bg-third/20">
              <IoCheckmarkOutline className="text-primary" size={22} />
          ...
          <Separator />
          <Button variant="third" className="py-4" onClick={disconnect}>
    ...
    <DialogContent className="max-h-screen overflow-auto">
      <AboutAptosConnect renderEducationScreen={renderEducationScreen}>
        ...
        <div className="flex flex-col gap-2 pt-10">
          <Typography>
            By connecting a wallet, you agree to Rigen&apos;s{" "}
            <Link
          ...
          {!!installableWallets.length && (
            <Collapsible className="flex flex-col gap-3">
              <CollapsibleTrigger asChild>
                <Button size="sm" variant="none" className="gap-2">
