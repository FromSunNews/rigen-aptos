...
import { FarmingPoolSlice, FarmingPoolState, FarmingPoolUI } from "@/store/types/farming-pool.type";
...
const poolCalculation = (pool: FarmingPoolUI) => {
  ...
  const baseBorrowingInterest =
    baseBorrowingRate.find((token) => token.symbol === pool.borrowTokens.selected)?.borrowingRate || 0;
    ...
    try {
      ...
      set((state: FarmingPoolSlice) => ({
        farmingPoolState: {
          ...
          filteredPools: state.farmingPoolState.pools.filter(
            (pool: FarmingPoolUI) => pool.platform.toLowerCase() === get().farmingPoolState.platformSelected
          ),
        },
      }));
    } catch (error) {
  ...
  updateLeverage: (selectedLeverage: number, poolId: string) => {
    ...
    set((state: FarmingPoolSlice) => ({
      farmingPoolState: {
        ...
        pools: updatedPools,
        // filteredPools will be updated automatically according to pools
        filteredPools: updatedPools.filter(
          (pool: FarmingPoolUI) => pool.platform.toLowerCase() === get().farmingPoolState.platformSelected
        ),
      },
    }));
